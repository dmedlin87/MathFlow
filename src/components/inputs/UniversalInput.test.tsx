...
// --- Smart Enter Tests (Added by Palette) ---

describe("UniversalInput - Smart Enter (Fraction)", () => {
  it("moves focus to denominator on Enter if denominator is empty", () => {
    const mockItem = createMockItem("fraction");
    const onChange = vi.fn();
    const onSubmit = vi.fn();
    ...
    // Spy on focus
    const focusSpy = vi.spyOn(denInput, "focus");
  ...
  it("submits on Enter if denominator is NOT empty", () => {
    const mockItem = createMockItem("fraction");
    const onChange = vi.fn();
    const onSubmit = vi.fn();
