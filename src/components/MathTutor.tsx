...
export const MathTutor: React.FC<MathTutorProps> = ({
  learnerState,
  setLearnerState,
  learnerService,
}) => {
  ...
  const handleSubmit = useCallback(
    async (e?: React.FormEvent, overrideAnswer?: string) => {
      ...
      try {
        if (!isCorrect) {
          ...
          try {
            const diagnosisResult = await learnerService.diagnose(
              currentItem,
              answerToCheck,
            );
            if (diagnosisResult) {
        ...
        const newState = await learnerService.submitAttempt(
          learnerState,
          attempt,
        );
        ...
        if (attempts === 0) {
          setSessionStats((prev) => ({
            ...
            masteredSkills:
              newState.skillState[currentItem.meta.skill_id]?.masteryProb > 0.85
                ? Array.from(
                    new Set([
                      ...prev.masteredSkills,
                      currentItem.meta.skill_id,
                    ]),
                  )
                : prev.masteredSkills,
          }));
          ...
          if (
            ...
            setSessionStats((prev) => ({
              ...prev,
              masteredSkills: Array.from(
                new Set([...prev.masteredSkills, currentItem.meta.skill_id]),
              ),
            }));
          }
        }
  ...
  // Optimization: Memoize steps to prevent breaking InteractiveSteps' React.memo
  const memoizedSteps = useMemo(() => {
    if (!currentItem?.solution_logic?.steps) return [];
  ...
  // Optimization: Stable input change handler to prevent UniversalInput re-renders
  ...
  // Optimization: Stable submit wrapper (though handleSubmit is already stable, this clarifies intent for prop)
  ...
  const isFractionEquivVars = (
    v: unknown,
  ): v is { baseNum: number; baseDen: number } => {
    if (typeof v !== "object" || v === null) return false;
    const r = v as Record<string, unknown>;
    return typeof r.baseNum === "number" && typeof r.baseDen === "number";
  };

  const fracEquivConfig =
    currentItem.meta.skill_id === "frac_equiv_01" && isFractionEquivVars(vars)
      ...
      <div className="mb-4 flex justify-between items-center text-sm text-gray-500">
        ...
        <div className="flex gap-4">
          <span>
            Ma:{" "}
            {(
              learnerState?.skillState[currentItem.meta.skill_id]
                ?.masteryProb || 0
            ).toFixed(2)}
          </span>
          {isLoading && (
            <span className="text-blue-500 font-bold text-xs animate-pulse">
              SYNCING...
            </span>
          )}
          <button
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="text-2xl font-medium mb-8 text-gray-800 flex flex-col items-center">
            ...
            {currentItem.problem_content.visual_spec && (
              <div className="mt-8 flex justify-center w-full">
                <ProblemVisualizer
                  spec={currentItem.problem_content.visual_spec}
                />
              </div>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {feedback !== "correct" &&
              currentItem.answer_spec.input_type !== "multiple_choice" && (
                <button
                  type="submit"
                  disabled={isLoading}
          ...
          {currentItem.solution_logic.steps && feedback === "incorrect" && (
            <InteractiveSteps steps={memoizedSteps} />
          )}
        </motion.div>
      </AnimatePresence>
    </div>
