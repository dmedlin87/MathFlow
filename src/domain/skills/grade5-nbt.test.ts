...
describe("Grade 5 NBT Domain", () => {
    ...
    describe("SKILL_5_NBT_MULT_DECIMALS", () => {
      it("generates valid decimal multiplication", async () => {
        for (let i = 0; i < 5; i++) {
          ...
          expect(
            parseFloat(problem.solution_logic.final_answer_canonical)
          ).toBeGreaterThan(0);
        }
      });
    });
  ...
  describe("SKILL_5_NBT_FRAC_DEC_CONV", () => {
    it("generates valid conversion problems", async () => {
      for (let i = 0; i < 10; i++) {
        const problem = await generate(SKILL_5_NBT_FRAC_DEC_CONV.id);
        expect(problem.meta.skill_id).toBe(SKILL_5_NBT_FRAC_DEC_CONV.id);
        const inputType = problem.answer_spec.input_type;
        expect(["decimal", "fraction"]).toContain(inputType);

        if (inputType === "decimal") {
          expect(problem.problem_content.stem).toContain(
            "Convert the fraction"
          );
        } else {
          expect(problem.problem_content.stem).toContain("Convert the decimal");
        }
      }
    });
  });
