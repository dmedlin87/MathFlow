...
export function checkAnswer(userAnswer: string, item: MathProblemItem): boolean {
    ...
    if (item.answer_spec.accepted_forms) {
        if (item.answer_spec.accepted_forms.some(form => String(form).trim() === normalizedUser)) {
            return true;
        }
    }
